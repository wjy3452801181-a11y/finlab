...
core/           跨模块共享的基础设施
  ...
  vocabulary.py 统一关键词注册表 — 所有宏观指标的多维分类单一数据源
  jin10.py      金十 MCP 适配器（单例客户端 + 格式化函数）
  config.py     用户配置管理（~/.config/finlab/config.toml）
macro/          宏观模块 — 经济日历拉取、评分、报告生成
  fetchers.py   经济日历 + ForexLive 新闻（TradingEconomics API）
  director.py   宏观总监（MacroDirector — 数据注入 + 四章节渲染）
ashare/         A股模块 — 数据、追踪、板块扫描、板块轮动
  data.py       Baostock 封装 + HistoricalSource 适配器
  chief.py      行业首席（SectorChief — 数据注入 + 多视角渲染）
  tracker.py    持仓追踪（TrackConfig/TrackResult 数据类）
  screener.py   板块扫描（scan_sectors/print_sector_scan 旧版入口）
trade/          交易模块 — 信号、风控、反共识Alpha
  executor.py   交易执行官（TradeExecutor — 数据注入 + 多视角渲染）
news/           新闻模块 — 快讯、分析、简报
...
- 指标分类规则从 `core/vocabulary.py` 统一注册表查询

**数据源适配器 (Data Source Adapter)**
每个外部数据源通过适配器满足对应接口。
- `Jin10QuoteAdapter`（`core/jin10.py`）— 金十实时报价
- `BaostockAdapter`（`ashare/data.py`）— A股历史日线
- 接口定义在调用方模块（如 report/sections.py 的 `MacroSource` Protocol）
